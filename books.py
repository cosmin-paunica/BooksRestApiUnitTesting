...
def get_books_by_name(title):
    conn = get_db_connection()
    book_rows = conn.execute('SELECT id, title, publish_date, genre FROM books WHERE title = (?)', (title)).fetchall()
...
def modify_book(id):
    conn = get_db_connection()
    data = request.get_json(force=True)
    if 'new_title' not in data or 'new_genre' not in data or 'new_publish_date' not in data:
        raise('invalid data')
    try:
    ...
    conn.execute('UPDATE books SET title = ?, genre = ?, publish_date = ? WHERE id = ?', (new_title, new_genre, new_publish_date, id))
    conn.commit()
    conn.close()
